...
void Plot(const Body& body, Screen& scr) { scr.PlotCircle(body.pos.x, body.pos.y, body.r); }

int main()
{
	Screen scr(0, 0, 10);

	constexpr float dt = 1.0f / 36.0f;
	...
	Earth.pos = {R, 0.0f};
	Earth.vel = {0.0f, std::sqrt(Sun.m / R)};

	Moon.pos = {R, r};
	Moon.vel = {std::sqrt(Earth.m / r), Earth.vel.y};
